...
require 'jsonEsperadoDeleteAll.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoDelete($mensajeRecibido)){

		$usuario = $mensajeRecibido["usuarioEliminarAll"];


		$query  = "TRUNCATE TABLE  inicio ";
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			...
			$arrMensaje["mensaje"] = "Usuarios eliminados correctamente";
