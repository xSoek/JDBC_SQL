...
require 'jsonEsperadoUpdate.php';
...
if(isset($parameters)){
	...
	if(JSONCorrectoAnnadir($mensajeRecibido)){
		...
		$usuario = $mensajeRecibido["usuarioUpdate"];

		$id = $usuario["id"];
		$nombre = $usuario["nombre"];
		$numero = $usuario["numero"];

		$query  = "UPDATE inicio SET Nombre = '$nombre', Numero = $numero WHERE Id = '$id'";
		...
		if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se ha insertado correctamente
			...
			$arrMensaje["mensaje"] = "Usuario modificado correctamente";
